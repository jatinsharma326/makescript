...
export const OverlayContextMenu: React.FC<OverlayContextMenuProps> = ({
    ...
    // Auto-generate initial AI image based on segment text
    const [autoGeneratedImage, setAutoGeneratedImage] = useState<{ imageUrl: string; prompt: string } | null>(null);

    // Generate AI image on mount if segment text exists
    React.useEffect(() => {
        if (segmentText && !existingOverlayType) {
            generateAutoImage();
        }
    }, [segmentText]);
    ...
    const generateAutoImage = async () => {
        try {
            // Use the segment text as the prompt
            const prompt = segmentText || 'professional business visualization';
            const encodedPrompt = encodeURIComponent(`${prompt}, cinematic professional style, high quality, dark moody background with dramatic lighting`);
            const imageUrl = `https://image.pollinations.ai/prompt/${encodedPrompt}?width=1024&height=768&nologo=true&seed=${Date.now()}`;
            setAutoGeneratedImage({ imageUrl, prompt });
        } catch (e) {
    ...
    const overlayOptions: { type: OverlayType; label: string; color: string; description: string }[] = [
        // Professional After Effects style overlays
